...
class CustomisationCommands(commands.Cog):
    ...
    async def feature_bird(self, interaction: discord.Interaction, bird_name: str):
        ...
        await interaction.response.send_message(f"✨ Your nest now features the {target_bird['common_name']} ({target_bird['scientific_name']})!")
        ...
        if entity_type == 'nest':
            ...
            if entity_type == 'bird':
                ...
                if not target_entity:
                    await interaction.response.send_message(f"Bird '{entity_name}' not found.", ephemeral=True)
                    return
                ...
                if not target_entity:
                    await interaction.response.send_message(f"Plant '{entity_name}' not found.", ephemeral=True)
                    return
    ...
    async def cleaning_handler(self, interaction: discord.Interaction, entity_type: str, entity_name: str = None):
        ...
        if entity_type == 'nest':
            ...
            await interaction.response.send_message(f"Removed {len(nest_treasures)} decorations from your nest. They have been returned to your inventory.", ephemeral=True)
            return
        ...
        if entity_type == 'bird':
            ...
            if not bird_treasures:
                await interaction.response.send_message(f"{target_entity['common_name']} has no decorations to remove.", ephemeral=True)
                return
            ...
            await interaction.response.send_message(f"Removed {len(bird_treasures)} decorations from {target_entity['common_name']}. They have been returned to your inventory.", ephemeral=True)
            ...
            await interaction.response.send_message(f"Removed {len(plant_treasures)} decorations from {target_entity['common_name']}. They have been returned to your inventory.", ephemeral=True)
