...
from data.models import get_remaining_actions, record_actions, load_bird_species
from config.config import SPECIES_IMAGES_DIR, DATA_PATH
from utils.logging import log_debug


MILESTONE_THRESHOLDS = [300,750,1500,3000,6000,12000,24000,48000,67000,100000,150000,210000,280000,380000,500000]
...
class ResearchCommands(commands.Cog):
    ...
    async def graduate_bird(self, interaction: discord.Interaction, bird_name: str):
        ...
        bird_count = len(remaining_birds)
        ...
        embed.add_field(
            ...
            value=f"You now have {bird_count} {'bird' if bird_count == 1 else 'birds'} in your nest.",
            inline=False
        )
    ...
    async def study(self, interaction: discord.Interaction, actions: int):
        ...
        released_birds = await db.get_released_birds()
        released_birds_bonus = 0
        released_birds_text = ""
        if released_birds:
            total_released_birds = sum(bird.get("count", 1) for bird in released_birds)
        ...
        async def select_callback(select_interaction):
            ...
            # Calculate bonus from released birds
            released_birds_bonus = 0
            released_birds_data = await db.get_released_birds()
            total_released_birds = 0
            if released_birds_data:
                # Sum up all counts in released_birds
                total_released_birds = sum(bird.get("count", 1) for bird in released_birds_data)
            ...
            # Increment research progress for the correct author
            await db.increment_research(correct_author, points_earned)

            # Load updated research progress
            research_progress = await db.load_research_progress()
            ...
            current_progress = research_progress.get(correct_author, 0)
            ...
            public_embed.add_field(
                ...
                value=f"**{research_progress.get(correct_author, 0)}** points",
                inline=True
            )
